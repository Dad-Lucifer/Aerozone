...
export default function App() {
  ...
  return (
    <div className={`relative min-h-screen w-full overflow-hidden bg-background text-foreground flex flex-col items-center justify-center font-sans p-4 ${isDarkMode ? 'dark' : ''}`}>
      ...
      {/* Dark/Light mode toggle button - Moved to avoid conflict with Navbar2 */}
      <button
        onClick={() => setIsDarkMode(!isDarkMode)}
        className="btn-toggle fixed top-4 right-4 z-40 p-2 rounded-full bg-secondary text-secondary-foreground shadow-md hover:bg-primary hover:text-primary-foreground transition-all duration-300"
      >
        {isDarkMode ? (
